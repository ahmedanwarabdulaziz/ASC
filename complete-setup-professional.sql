-- ============================================
-- PROFESSIONAL SUPABASE SETUP
-- Optimized for Fast Arabic Search
-- ============================================
...
-- GIN index for full-text search (fastest for Arabic)
CREATE INDEX IF NOT EXISTS members_search_vector_idx ON members USING GIN (search_vector);

-- GIN index for array search (fast prefix matching)
CREATE INDEX IF NOT EXISTS members_search_tokens_idx ON members USING GIN (search_tokens);
CREATE INDEX IF NOT EXISTS members_name_tokens_idx ON members USING GIN (name_tokens);
CREATE INDEX IF NOT EXISTS members_address_tokens_idx ON members USING GIN (address_tokens);
CREATE INDEX IF NOT EXISTS members_job_tokens_idx ON members USING GIN (job_tokens);
...
-- Text search indexes for ILIKE queries (backup search method)
CREATE INDEX IF NOT EXISTS members_name_search_idx ON members (name_search text_pattern_ops);
CREATE INDEX IF NOT EXISTS members_address_search_idx ON members (address_search text_pattern_ops);
CREATE INDEX IF NOT EXISTS members_job_search_idx ON members (job_search text_pattern_ops);
        ...
        translate(
          translate(
            translate(input_text, 
              'أإآا', 'ا'), -- Normalize Alef
            'ة', 'ه'), -- Normalize Teh Marbuta
          'ي', 'ي'), -- Normalize Yeh
        'ى', 'ي'), -- Normalize Alef Maksoora
      'ًٌٍَُِّْ', '' -- Remove diacritics
    )
  );
END;
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      -- Full-text search rank (highest weight)
      ts_rank(m.search_vector, plainto_tsquery('simple', normalized_query)) * 10,
      0
    ) +
    COALESCE(
      -- Array contains rank (medium weight)
      CASE WHEN m.search_tokens && query_tokens THEN 
        array_length(array(SELECT unnest(m.search_tokens) INTERSECT SELECT unnest(query_tokens)), 1) * 5.0
      ELSE 0 END,
      0
    ) +
    COALESCE(
      -- Prefix match rank (lower weight)
      CASE WHEN EXISTS (
  ...
  FROM members m
  WHERE 
    -- Method 1: Full-text search (fastest for Arabic)
    ...
    -- Method 3: ILIKE search (backup for partial matches)
